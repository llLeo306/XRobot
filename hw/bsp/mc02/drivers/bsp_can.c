...
void bsp_can_init(void) {
  ...
  if (HAL_FDCAN_ConfigFilter(&hfdcan3, &can_filter) != HAL_OK) {
    XB_ASSERT(false);
  }
...
bool bsp_can_check_error(uint32_t *error_code, bsp_can_t *can) {
  *error_code = HAL_FDCAN_ERROR_NONE;
  *error_code = HAL_FDCAN_GetError(&hfdcan1);
  if (*error_code & ~(HAL_FDCAN_ERROR_NONE | HAL_FDCAN_ERROR_FIFO_EMPTY |
  ...
  *error_code = HAL_FDCAN_GetError(&hfdcan2);
  if (*error_code & ~(HAL_FDCAN_ERROR_NONE | HAL_FDCAN_ERROR_FIFO_EMPTY |
                      HAL_FDCAN_ERROR_FIFO_FULL)) {
    hfdcan2.ErrorCode &= (HAL_FDCAN_ERROR_NONE);
    *can = BSP_CAN_2;
    return true;
  }

  *error_code = HAL_FDCAN_GetError(&hfdcan3);
  if (*error_code & ~(HAL_FDCAN_ERROR_NONE | HAL_FDCAN_ERROR_FIFO_EMPTY |
                      HAL_FDCAN_ERROR_FIFO_FULL)) {
...
// void bsp_can_reset(bsp_can_t can) {
//   (bsp_can_get_handle(can));
// }

void bsp_can_get_state(bsp_can_t can, HAL_FDCAN_StateTypeDef *state) {
  *state = HAL_FDCAN_GetState(bsp_can_get_handle(can));
}
