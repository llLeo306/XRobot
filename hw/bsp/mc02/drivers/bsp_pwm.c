...
static void ws2812_ctrl() {
  ...
  HAL_SPI_Transmit(&hspi6, spi_tx_buff, sizeof(spi_tx_buff), 1);
  memset(ws2812_color_grb, 0, sizeof(ws2812_color_grb));
}
...
bsp_status_t bsp_pwm_set_comp(bsp_pwm_channel_t ch, float duty_cycle) {
  ...
  if (ch >= BSP_PWM_LED_GRN && ch <= BSP_PWM_LED_BLU) {
    ws2812_color_grb[ch - BSP_PWM_LED_GRN] =
        (uint8_t)(duty_cycle / 10 * 255.0f);
    ws2812_ctrl();
    return BSP_OK;
  }
...
bsp_status_t bsp_pwm_set_freq(bsp_pwm_channel_t ch, float freq) {
  uint16_t reload = (uint16_t)(275000000U / (freq * 1000) - 1);
